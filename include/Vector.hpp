...
namespace ft
{
    ...
    public:
        ...
        {
            ft::swap(start, rhs.start);
            ft::swap(finish, rhs.finish);
            ft::swap(endOfStorage, rhs.endOfStorage);
            allocator_type tmp;
            tmp = rhs.alloc;
    ...
    protected:
        ...
        {
            size_type len = checkLen(size_type(1), "vector::reallocInsert");
        ...
        void initializeDispatch(Integer n, Integer value, ft::true_type)
        {
            ...
            for (size_type i = 0; i < static_cast<size_type>(n); i++)
                construct(Base::start, value, i);
            Base::finish = Base::start + static_cast<size_type>(n);
            Base::endOfStorage = Base::finish;
        }
        ...
        {
            Base::start = allocate(last - first);
            Base::finish = Base::start;
            Base::endOfStorage = Base::start + (last - first);
        ...
        ///
        template <typename U>
        U *dataPtr(U *ptr) const { return ptr; }
        template <typename U>
        U *dataPtr(U *ptr) { return ptr; }
        template <typename Ptr>
        value_type *dataPtr(Ptr ptr)
        {
            return empty() ? (value_type *)0 : ptr.operator->();
        }
        template <typename Ptr>
        const value_type *dataPtr(Ptr ptr) const
        {
